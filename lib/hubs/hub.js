...
module.exports = function (socket, io) {
    ...
    socket.on('game:seat', function (data, fn) {
        console.log('game:seat received: ' + socket.userName);
        if (!game.seatPlayer(socket.userName)) {
            return 'no seats available in this room';
        }

        console.log('game:seat to room: ' + socket.room + '; positoin: ' + game.playerPosition(socket.userName));
        ...
        if (game.gameStarted()) {
            console.log('game:seat game started');
            game.startRound();
    ...
    // user seat in the position
    socket.on('game:restart', function (data, fn) {
        console.log('game:restart received: ' + socket.userName);
        if (game.gameStarted()) {
            console.log('game:restart game started');
            game.startRound();
            io.sockets.in(socket.room).emit('game:roundstarted', {
                deck: game.round.playerDecks
            });
